...
==>fail::[inst apply failure: -2 is not a int::T[is(gt(0))][choose([is(lt(0))=>mult(-1),id()=>id()]).rshift()]@/m/math/nat [structural]]@/sys/fail/3192
```
...
==>fail::[inst apply failure: human::[name=>'marko',age=>29] is not a being::T@chicken [nominal]]@/sys/fail/3196
```
...
==>fail::[inst apply failure: -2 is not a int::T[is(gt(0))][choose([is(lt(0))=>mult(-1),id()=>id()]).rshift()]@/m/math/nat [structural]]@/sys/fail/3220
mtron> [-- Without a constructor, .as() is a pure test --]
mtron> -2.as(int::T[?>0])  [-- fails: no constructor to rescue --]
==>fail::[inst apply failure: -2 is not a int::T[is(gt(0))] [structural]]@/sys/fail/3224
```
...
==>fail::[inst apply failure: human::[name=>'marko',age=>29] is not a being::T@chicken [nominal]]@/sys/fail/3228
```
