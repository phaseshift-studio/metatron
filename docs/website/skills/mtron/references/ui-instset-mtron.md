...
mtron> */usr/uidoc/panel/title                [-- one key --]
==>'note'
...
```mtron
mtron> panel_widget::[title=>'note',body=>'alpha'].as(str::T)     [-- the rec, quoted: not the render --]
==>"{title='note', body='alpha', style=style::[border=>┌;┐;└;┘;│;│;─;─;┬;┴;├;┤]}"
```
