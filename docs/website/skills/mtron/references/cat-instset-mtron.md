...
==>fail::[inst apply failure: plus?rng=int&dom=int(int::T) is not a morphism::T [nominal] (at /m/inst/as@1)]@/sys/fail/622
```
...
==>fail::[inst apply failure: plus?rng=int&dom=int(int::T) is not a morphism::T [nominal] (at /m/inst/as@1)]@/sys/fail/650
```
