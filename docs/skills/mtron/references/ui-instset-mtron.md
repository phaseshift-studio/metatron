...
```mtron_pre
tree_widget::[root=>/m/mach/ui, max=>2].as?str<=widget(str::T)
```
...
```mtron_pre
*/m/mach/ui/widget/panel_widget       [-- title and body, both optional; ctor?panel_widget<=#{?} --]
*/m/mach/ui/widget/tree_widget        [-- root and max required; code/flatten/xref/expand optional --]
```
...
```mtron_pre
accordion_widget::[title=>'notes',/
                   body=>"l01\nl02\nl03"]@/usr/uidoc/notes
@/usr/uidoc/notes >>= [body=>"l01\nl02\nl03\nl04"]              [-- the update carries the insts with it --]
*/usr/uidoc/notes                                               [-- the widget, as the rec it is --]
```
...
```mtron_pre
@/usr/uidoc/panel >>= [style=>[anchor=>top_left,width=>20,foreground=>'{{c}}']]
*/usr/uidoc/panel/style
```
...
```mtron_pre
@/usr/uidoc/panel.as?str<=widget(str::T)
```
...
```mtron_pre
[MAXOUTPUT 8] table_widget::[header=>['name','qty'],row=>[['alpha','3'],['beta','12']]].as?str<=widget(str::T)
```

```mtron_pre
[MAXOUTPUT 10] tree_widget::[root=>/m/mach/ui, max=>1].as?str<=widget(str::T)
```
...
```mtron_pre
panel_widget::[title=>'note',body=>'alpha beta gamma delta epsilon zeta',style=>[width=>24]].as?str<=widget(str::T)
```
