...
```mtron_pre
*as?str<=int&asq=[coupling]
```

```mtron_pre
*as?str<=int&asq=[duplicate]
```
...
```mtron_pre
'{"a":1}'.as(json::T)                                 [-- a tag: the generic arg-type cast --]
```

```mtron_pre
'{"a":1}'.as(json::T).as(rec::T)                      [-- the tag, then the declared parse cast --]
```
