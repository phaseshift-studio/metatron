...
/**
 * The transient frame provider — pops the frame complete and then clears it, so the frame tree
 * is an ephemeral call stack rather than durable compute memory.
 */
public class TransientFrameFeatureTest extends AbstractMetatronTest {
    ...
    @Test
    public void testFrameLifecycleTransient() {
        ...
        final TransientFrameFeature frame = new TransientFrameFeature(
                mutableMap(uri(ROOT), uri(ROOT)), LLM_TRANSIENT_FRAME_FEATURE_TID, null);
        ...
        // pop marks complete AND clears the frame (ephemeral, not durable)
        ...
        final Obj cleared = Router.readFromSpace(frameURI);
