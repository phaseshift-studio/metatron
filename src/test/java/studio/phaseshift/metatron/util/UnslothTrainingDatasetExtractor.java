...
public class UnslothTrainingDatasetExtractor {
    ...
    public static void main(String[] args) {
        ...
        List<Training.Entry> dataset = new ArrayList<>();
        ...
        for (String className : targetClasses) {
            try {
                ...
                ClassStats stats = extractFromClass(clazz, dataset, ignoreTestMethods);
                allStats.add(stats);
    ...
    private static void addMetaKnowledge(List<Training.Entry> dataset) {
        ...
        try {
            ...
            for (final Sugar s : isa.sugars()) {
                ...
                dataset.add(new Training.Entry(SUGAR_TO, sugarEx, desugEx, null));
                // Training pair: desugar → sugar (skip for multi-inst chains — too ambiguous)
                if (s.getInstChain().size() == 1) {
                    dataset.add(new Training.Entry(SUGAR_FROM, desugEx, sugarEx, null));
                }
            }
    ...
    private static ClassStats extractFromClass(Class<?> clazz, List<Training.Entry> dataset, Set<String> ignoreTestMethods) {
        ...
        for (Method method : clazz.getDeclaredMethods()) {
            ...
            if (method.isAnnotationPresent(ParameterizedTest.class) && !method.isAnnotationPresent(TestData.class)) {
                ...
                if (csv != null) {
                    List<Training.Entry> entries = Training.Extractor.from(method, csv);
                    dataset.addAll(entries);
                    rowsExtracted += entries.size();
                }
            }
        }
    ...
    private static void writeJsonl(List<Training.Entry> entries, String path) {
        try (FileWriter writer = new FileWriter(path)) {
            String lastSourceMethod = null;
            for (Training.Entry entry : entries) {
